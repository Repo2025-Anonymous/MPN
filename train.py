...
def parse_args():
    ...
    parser.add_argument('--dataset',  type=str,  default="deepglobe", choices=['isic', 'deepglobe', 'isic', 'lung'], help='training dataset')
    parser.add_argument('--batch-size', type=int,  default=20, help='batch size of training')
...
def evaluate(model, dataloader, args):
    ...
    for i, (img_s_list, mask_s_list, img_q, mask_q, cls, _, id_q) in enumerate(tbar):
        ...
        with torch.no_grad():
            query_out = model(img_s_list, mask_s_list, img_q, mask_q)
            pred = torch.argmax(query_out, dim=1)
...
def main():
    ...
    save_path = './outdir/PMN/%s/train' % (args.dataset)
    ...
    for epoch in range(args.episode // args.snapshot):
        ...
        for i, (img_s_list, mask_s_list, img_q, mask_q, _, _, _) in enumerate(tbar):
            ...
            query_out = model(img_s_list=img_s_list, mask_s_list=mask_s_list, img_q=img_q, mask_q=mask_q)
            loss = loss_ce(query_out, mask_q)            
